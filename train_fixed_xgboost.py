...
"""
房價預測競賽 - 修正版XGBoost模型
House Price Prediction Competition - Fixed XGBoost Model
"""
...
def main():
    ...
    plt.subplot(1, 2, 1)
    sns.histplot(y_train_original, kde=True)
    plt.title('原始銷售價格分佈')
    plt.xlabel('銷售價格')
    ...
    # 儲存模型
    model_path = f'{model_dir}/fixed_xgboost_model.pkl'
    ...
    # 儲存特徵重要性
    feature_importance.to_csv(f'{model_dir}/fixed_xgboost_feature_importance.csv', index=False)
    ...
    plt.xlabel('重要性分數')
    plt.tight_layout()
    plt.savefig(f'{model_dir}/fixed_xgboost_top_features.png', dpi=300)
    ...
    plt.ylabel('頻率')
    plt.tight_layout()
    plt.savefig(f'{model_dir}/fixed_xgboost_predicted_distribution.png', dpi=300)
    ...
    # 儲存提交文件
    submission_path = f'{model_dir}/fixed_xgboost_submission.csv'
