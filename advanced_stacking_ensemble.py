...
"""
房價預測競賽 - 高級多層堆疊集成模型
House Price Prediction Competition - Advanced Multi-layer Stacking Ensemble Model
==============================================================================

此腳本實現高級多層堆疊集成方法，通過以下技術提高預測性能：
1. 多層堆疊架構 - 實現三層堆疊模型結構
2. 特徵重用 (Restacking) - 在堆疊過程中保留重要原始特徵
...
def get_feature_groups(X_train, top_n=30):
    """將特徵分成不同的組，用於異質化基礎模型"""
    ...
    feature_groups = {
        ...
        'even': [i for i in all_features if i % 2 == 0],  # 偶數索引特徵
        'odd': [i for i in all_features if i % 2 == 1],  # 奇數索引特徵
        'first_half': all_features[:n_features//2],  # 前半部分特徵
        'second_half': all_features[n_features//2:],  # 後半部分特徵
...
def get_base_models(use_different_features=True):
    ...
    # 樹模型組
    tree_models = [
        ('rf', RandomForestRegressor(n_estimators=150, max_depth=20, min_samples_split=5, min_samples_leaf=2, random_state=42, n_jobs=-1)),
        ('gbm', GradientBoostingRegressor(n_estimators=200, learning_rate=0.05, max_depth=5, min_samples_split=4, min_samples_leaf=2, random_state=42)),
        ('xgb', XGBRegressor(n_estimators=200, learning_rate=0.05, max_depth=5, min_child_weight=2, gamma=0.1, subsample=0.8, colsample_bytree=0.8, random_state=42)),
        ('lgbm', LGBMRegressor(n_estimators=200, learning_rate=0.05, max_depth=6, num_leaves=40, min_child_samples=5, subsample=0.8, colsample_bytree=0.8, random_state=42))
    ]
...
def get_meta_models():
    """定義元模型"""
    return {
        'linear': Ridge(alpha=0.1, random_state=42),
        'tree': XGBRegressor(n_estimators=150, learning_rate=0.03, max_depth=4, min_child_weight=2, gamma=0.1, subsample=0.8, colsample_bytree=0.8, random_state=42),
        'other': LGBMRegressor(n_estimators=150, learning_rate=0.03, max_depth=4, num_leaves=31, subsample=0.8, colsample_bytree=0.8, random_state=42),
        'final': XGBRegressor(n_estimators=250, learning_rate=0.02, max_depth=3, min_child_weight=2, gamma=0.05, subsample=0.8, colsample_bytree=0.8, reg_alpha=0.01, reg_lambda=0.01, random_state=42)
    }
...
def fit_meta_model(meta_features, y, meta_model, name='元模型'):
    ...
    try:
        ...
        # 使用之前的近似方法作為備選
        # 近似對數空間RMSE - 以固定係數0.000006轉換(基於房價範圍校準)
        # 這是一個估計值，實際值可能略有不同
...
def multi_layer_stacking(X_train, y_train, X_test, n_folds=5, use_restacking=True):
    ...
    # 如果沒有找到特定特徵，使用方差篩選
    if len(best_features_indices) < 10:
        # 使用方差作為特徵重要性的代理
        variances = X_train.var().values
        feature_indices = np.argsort(-variances)  # 降序排列
        best_features_indices.extend(feature_indices[:10])
        best_features_indices = list(set(best_features_indices))  # 去重
    
    # 截斷到前15個
    best_features_indices = best_features_indices[:15]
    ...
    if use_restacking:
        ...
        final_meta_features = np.hstack([
            ...
            all_first_layer_meta_features[:, best_model_indices],  # 最好的第一層模型預測
            X_train.iloc[:, best_features_indices].values  # 重要的原始特徵
        ])
        
        final_test_meta_features = np.hstack([
            second_layer_test_meta_features,
            all_first_layer_test_meta_features[:, best_model_indices],  # 最好的第一層模型預測
            X_test.iloc[:, best_features_indices].values  # 重要的原始特徵
        ])
    else:
    ...
    print(f"\n多層堆疊集成訓練耗時: {training_time:.2f} 分鐘")
    ...
    with open(f'{output_dir}/advanced_stacking_ensemble_model.pkl', 'wb') as f:
...
def plot_model_performance(first_layer_cv_scores, second_layer_cv_scores, final_meta_cv_rmse):
    ...
    # 設置圖表屬性
    plt.title('多層堆疊集成模型性能比較 (RMSE 對數空間)')
    plt.xlabel('模型')
    ...
    # 保存圖表
    plt.savefig(f'{output_dir}/advanced_stacking_model_performance.png', dpi=300)
    ...
    # 保存圖表
    plt.savefig(f'{output_dir}/advanced_stacking_model_performance_grouped.png', dpi=300)
    plt.close('all')
...
def generate_submission(final_pred, test_ids):
    ...
    # 保存提交文件
    submission.to_csv(f'{output_dir}/advanced_stacking_ensemble_submission.csv', index=False)
    print(f"提交文件已保存到 {output_dir}/advanced_stacking_ensemble_submission.csv")
    
    # 繪製預測分佈圖
    plt.figure(figsize=(10, 6))
    sns.histplot(final_pred, kde=True)
    plt.title('高級堆疊集成: 預測分佈 (對數空間)')
    plt.xlabel('預測值 (對數)')
    plt.ylabel('頻率')
    plt.savefig(f'{output_dir}/advanced_stacking_ensemble_predicted_distribution.png', dpi=300)
    plt.close()
...
def main():
    ...
    final_pred, meta_cv_rmse, training_time = multi_layer_stacking(
        X_train, y_train, X_test, n_folds=3, use_restacking=True
    )
    ...
    # 更新摘要HTML文件
    update_summary_html("高級多層堆疊集成", meta_cv_rmse, training_time)
