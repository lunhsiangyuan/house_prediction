...
"""
房價預測競賽 - 隨機森林模型
House Price Prediction Competition - Random Forest Model
"""
...
def main():
    ...
    # 儲存特徵重要性
    feature_importance.to_csv(f'{model_dir}/random_forest_feature_importance.csv', index=False)
    ...
    plt.title('隨機森林: Top 20 特徵重要性')
    plt.xlabel('重要性分數')
    plt.tight_layout()
    plt.savefig(f'{model_dir}/random_forest_top_features.png', dpi=300)
    ...
    plt.title('隨機森林預測銷售價格分佈')
    plt.xlabel('預測銷售價格')
    plt.ylabel('頻率')
    plt.tight_layout()
    plt.savefig(f'{model_dir}/random_forest_predicted_distribution.png', dpi=300)
    ...
    # 儲存提交文件
    submission_path = f'{model_dir}/random_forest_submission.csv'
